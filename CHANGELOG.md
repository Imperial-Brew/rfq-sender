...
### Fixed
- Duplicate Part Numbers with Different Processes: Fixed an issue where drafting emails or creating Box folders for a part with multiple required finishes (e.g., Chromate and Anodize) would always select the first entry in the queue. The system now uniquely identifies queue items using both part number and process name.
